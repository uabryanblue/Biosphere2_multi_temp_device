...
def readthermocouple(CSPin, spi):
    raw_data = bytearray(4)
    cs = machine.Pin(CSPin, machine.Pin.OUT)
...
def ReadThermocouples(readings):
    ...
    for key in readings.keys():
        CSPin = key
        readings[key] = readthermocouple(CSPin, spi)

    # CSPin = 16 # and 0 work right now
    callibration = []
    # temp1 = readthermocouple(CSPin, spi)
    # temp1_corrected = 0.0
    
    # CSPin = 0
    # temp2 = readthermocouple(CSPin, spi)
